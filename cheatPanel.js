(function () {
    ...
    // Call exposeCheats() if available, then wait for player/items
    function tryExpose() {
        if (window.exposeCheats) window.exposeCheats()
    }
    ...
    function act(fn) {
        ready(() => {
            fn()
            try { if (p().updateInventory) p().updateInventory() } catch(e){}
            updateDisplays()
        })
    }
    ...
    style.textContent = `
        ...
        #bssPanel h2 { margin:0 0 14px;font-size:16px;color:#f5a623;display:flex;justify-content:space-between;align-items:center; }
        #bssPanel .section { margin-bottom:12px; }
        #bssPanel .label { color:#f5a623;font-weight:600;margin-bottom:6px;font-size:12px; }
        ...
        .bssBtn { background:#7a4a00;border:none;border-radius:7px;color:#fff;padding:5px 11px;cursor:pointer;font-size:12px; }
        .bssBtn:hover { background:#f5a623;color:#111; }
        .bssBtn.red { background:#5a1a1a; }
        .bssBtn.red:hover { background:#c0392b; }
        #bssAutoAmt { width:120px;background:#2a1a00;border:1px solid #f5a623;border-radius:6px;color:#fff;padding:4px 6px;font-size:12px; }
        #bssStatus { background:#1a2a1a;border-radius:6px;padding:5px 8px;font-size:11px;color:#7f7;margin-bottom:10px;min-height:20px; }
        #bssPanel .hint { margin-top:12px;font-size:10px;color:#666;text-align:center; }
    `
    ...
    panel.innerHTML = `
        ...
        <div id="bssStatus">Waiting for game…</div>
        ...
        <div class="section">
            <div class="label">Pollen / Capacity <span class="sub" id="bssCapVal"></span></div>
            <div class="row">
        ...
        <div class="section">
            <div class="label">Auto Honey (added every second)</div>
            <div class="row">
    ...
    function updateDisplays() {
        const pl = p()
        if (!pl) { setStatus('Game not loaded yet — keep waiting…', '#fa0'); return }
        setStatus(
            'Honey: ' + Math.round(pl.honey).toLocaleString() +
            '  |  Pollen: ' + Math.round(pl.pollen || 0).toLocaleString(), '#7f7'
        )
    ...
    // Keep calling exposeCheats every 500ms until player exists
    function pollReady() {
        tryExpose()
    ...
    document.getElementById('bssPolConvert').onclick = () => act(() => {
        p().honey += Math.ceil((p().pollen || 0) * p().honeyPerPollen)
        p().pollen = 0
    })
    ...
    document.getElementById('bssGiveAll').onclick = () => act(() => {
        let hit = 0, miss = []
        RESOURCE_ITEMS.forEach(n => giveItem(n, GIVE_AMOUNT) ? hit++ : miss.push(n))
        setStatus('Gave ' + GIVE_AMOUNT + 'x to ' + hit + ' items' +
            (miss.length ? ' | Missing: ' + miss.slice(0,5).join(', ') + (miss.length>5?'…':'') : ''), '#7f7')
    })
    ...
    document.getElementById('bssGiveCustom').onclick = () => {
        ...
        act(() => {
            if (giveItem(name.trim(), amt))
                ...
                setStatus('❌ "' + name + '" not found — check spelling exactly', '#f77')
        })
    }
    ...
    document.getElementById('bssAutoToggle').onclick = function () {
        if (autoInterval) {
            ...
            autoInterval = setInterval(() => {
                if (p()) {
                    p().honey += parseFloat(document.getElementById('bssAutoAmt').value) || 1e9
                    try { if (p().updateInventory) p().updateInventory() } catch(e){}
                    updateDisplays()
                }
            }, 1000)
    ...
    document.addEventListener('keydown', e => {
        if (e.key === '`' || e.key === '~') {
            ...
            if (!showing) { tryExpose(); updateDisplays() }
        }
    })
